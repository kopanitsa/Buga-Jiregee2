...
public class PushServer {
	...
	private static void sendMessageByKey(List<String> playerKeys, Message msg) {
		...
		for (String player : playerKeys) {
			...
			try {
				...
				// TODO A bug in the dev_appserver causes an exception to be
				// thrown when
	...
	/**
	 * Create a channel for a player. Returns the channel id that the client
	 * must use to connect for receiving push messages.
	 * 
	 * @param player
	...
	public static String createChannel(Player player) {
		// String channelId =
		// getChannelService().createChannel(player.getKey());
		String channelId = getChannelService().createChannel(player.getName());
		logger.warning("Returning new channel " + channelId + " for player "
				+ player.getName());
		return channelId;
	}
